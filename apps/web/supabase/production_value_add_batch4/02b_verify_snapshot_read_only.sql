-- ============================================================================
-- PRODUCTION VALUE-ADD BATCH 4 — 02b VERIFY: READ-ONLY POSTCHECK NACH 02
-- ============================================================================
-- SICHERHEITSHINWEISE — vor dem Ausfuehren lesen:
--
--   1. Diese Datei laeuft AUSSCHLIESSLICH nach abgeschlossenem Schritt 02
--      (02_backup_value_add_batch4.sql, Ergebnis backup_rows = 12) und VOR
--      Schritt 03.
...
--   4. Bei IRGENDEINEM FAIL: Schritt 03 (03_backfill_value_add_batch4.sql)
--      NICHT starten. Befund melden und Ursache klaeren.
--   5. Fehlt cbb_private_backup.value_add_pre_backfill_v4, scheitert das
...
--   Der Batch-4-Snapshot wird direkt referenziert — sein Fehlen ist ein
--   Planungsfehler, kein Report. Die Artefakte von Batch 1, 2 und 3 werden
...
--   nicht. Der harte Beleg, dass alle drei Vorgaenger inhaltlich intakt sind,
...
-- ---------------------------------------------------------------------------
-- Snapshot-Zielmenge: exakt die 12 Slugs aus 02 (Pruefung 3)
-- ---------------------------------------------------------------------------
slug_set as (
  select
    (select count(*)
...
snapshot_shape as (
  ...
  from pg_attribute a
  where a.attrelid = 'cbb_private_backup.value_add_pre_backfill_v4'::regclass
    and a.attnum > 0
...
snapshot_guard as (
  ...
  from pg_class c
  where c.oid = 'cbb_private_backup.value_add_pre_backfill_v4'::regclass
),
...
table_privileges_effective as (
  ...
  where r.rolname in ('anon', 'authenticated')
    and has_table_privilege(
          r.oid,
          'cbb_private_backup.value_add_pre_backfill_v4'::regclass::oid,
          p.priv::text)
),
schema_privileges_effective as (
  ...
  join pg_class c
    on c.oid = 'cbb_private_backup.value_add_pre_backfill_v4'::regclass
  where r.rolname in ('anon', 'authenticated')
    and has_schema_privilege(r.oid, c.relnamespace::oid, p.priv::text)
),

-- ---------------------------------------------------------------------------
-- Batch-4-Payload aus 03 darf noch NICHT existieren (Pruefung 15).
-- Die Artefakte von Batch 1, 2 und 3 muessen noch da sein (Pruefung 16).
...
checks as (
  ...
  union all
  -- 1
  select 30, 'snapshot_v4_zeilen', zeilen::text, '12',
    case when zeilen = 12 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 2
  select 40, 'snapshot_v4_spalten',
  ...
  union all
  -- 3
  select 50, 'snapshot_v4_zielmenge',
  ...
  union all
  -- 4
  select 60, 'snapshot_v4_ids_slugs_eindeutig',
    eindeutige_ids::text || '/' || eindeutige_slugs::text,
    '12/12',
    case when eindeutige_ids = 12 and eindeutige_slugs = 12
      then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 5
  select 70, 'snapshot_v4_value_add_nicht_null', value_add_nicht_null::text, '0',
    case when value_add_nicht_null = 0 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 6
  select 80, 'snapshot_v4_gegen_products_drift', drift::text, '0',
  ...
  union all
  -- 9
  select 110, 'snapshot_v4_rls', rls::text, 'true',
    case when rls is true then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 10
  select 120, 'snapshot_v4_policies', policies::text, '0',
    case when policies = 0 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 11
  select 130, 'snapshot_v4_constraints',
  ...
  union all
  -- 13
  select 150, 'snapshot_v4_tabellenrechte_app_rollen',
  ...
  union all
  -- 14
  select 160, 'snapshot_v4_schemarechte_app_rollen',
    'Rollen ' || app_rollen::text || '/2'
  ...
  union all
  -- 15
  select 170, 'payload_v4_fehlt_noch', payload_v4_fehlt::text, 'true',
    case when payload_v4_fehlt then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 16
  ...
  select 190, 'vorgaenger_befuellung_intakt',
    befuellt_gesamt::text || ' Zeilen mit Value-Add gesamt'
      ...
      || ' | Batch 3 vollstaendig ' || batch3_vollstaendig::text,
  ...
  from summary
  union all
  select 200, 'snapshot_v4_bestehende_editorial_notes',
