-- ============================================================================
-- PRODUCTION VALUE-ADD BATCH 2 — 02b VERIFY: READ-ONLY POSTCHECK NACH 02
-- ============================================================================
-- SICHERHEITSHINWEISE — vor dem Ausfuehren lesen:
--
--   1. Diese Datei laeuft AUSSCHLIESSLICH nach abgeschlossenem Schritt 02
--      (02_backup_value_add_batch2.sql, Ergebnis backup_rows = 10) und VOR
--      Schritt 03.
...
--   4. Bei IRGENDEINEM FAIL: Schritt 03 (03_backfill_value_add_batch2.sql)
--      NICHT starten. Befund melden und Ursache klaeren.
--   5. Fehlt cbb_private_backup.value_add_pre_backfill_v2, scheitert das
...
--   Der Batch-2-Snapshot wird direkt referenziert — sein Fehlen ist ein
--   Planungsfehler, kein Report. Die BATCH-1-Artefakte werden dagegen nur ueber
--   to_regclass geprueft, damit ihr Fehlen als lesbare FAIL-Zeile erscheint
--   (Zeile 180) statt als "relation does not exist". Preis: die exakte
--   Zeilenzahl der v1-Tabellen liest diese Datei bewusst nicht. Der harte
--   Beleg, dass Batch 1 inhaltlich intakt ist, kommt aus public.products
--   (Zeile 190), nicht aus den v1-Tabellen.
--
...
snapshot_shape as (
  ...
  from pg_attribute a
  where a.attrelid = 'cbb_private_backup.value_add_pre_backfill_v2'::regclass
    and a.attnum > 0
...
-- ---------------------------------------------------------------------------
-- Live-Zustand (Pruefungen 7, 8) und Batch-1-Beleg (Pruefung 17)
-- ---------------------------------------------------------------------------
products_state as (
  select
...
table_privileges_effective as (
  ...
  where r.rolname in ('anon', 'authenticated')
    and has_table_privilege(
          r.oid,
          'cbb_private_backup.value_add_pre_backfill_v2'::regclass::oid,
          p.priv::text)
),
schema_privileges_effective as (
  ...
  join pg_class c
    on c.oid = 'cbb_private_backup.value_add_pre_backfill_v2'::regclass
  where r.rolname in ('anon', 'authenticated')
    and has_schema_privilege(r.oid, c.relnamespace::oid, p.priv::text)
),

-- ---------------------------------------------------------------------------
-- Batch-2-Payload aus 03 darf noch NICHT existieren (Pruefung 15).
-- Batch-1-Artefakte muessen noch da sein (Pruefung 16). Beides ueber
-- to_regclass, damit ein Befund als FAIL-Zeile erscheint (siehe Kopf).
-- ---------------------------------------------------------------------------
artefakt_state as (
  select
...
checks as (
  ...
  union all
  -- 1
  select 30, 'snapshot_v2_zeilen', zeilen::text, '10',
    case when zeilen = 10 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 2
  select 40, 'snapshot_v2_spalten',
  ...
  union all
  -- 3
  select 50, 'snapshot_v2_zielmenge',
  ...
  union all
  -- 4
  select 60, 'snapshot_v2_ids_slugs_eindeutig',
  ...
  union all
  -- 5
  select 70, 'snapshot_v2_value_add_nicht_null', value_add_nicht_null::text, '0',
    case when value_add_nicht_null = 0 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 6
  select 80, 'snapshot_v2_gegen_products_drift', drift::text, '0',
  ...
  union all
  -- 9
  select 110, 'snapshot_v2_rls', rls::text, 'true',
    case when rls is true then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 10
  select 120, 'snapshot_v2_policies', policies::text, '0',
    case when policies = 0 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 11
  select 130, 'snapshot_v2_constraints',
  ...
  union all
  -- 13
  select 150, 'snapshot_v2_tabellenrechte_app_rollen',
  ...
  union all
  -- 14
  select 160, 'snapshot_v2_schemarechte_app_rollen',
    'Rollen ' || app_rollen::text || '/2'
  ...
  union all
  -- 15
  select 170, 'payload_v2_fehlt_noch', payload_v2_fehlt::text, 'true',
    case when payload_v2_fehlt then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 16
  select 180, 'batch1_artefakte_vorhanden',
