-- ============================================================================
-- PRODUCTION VALUE-ADD BATCH 4 — 04b VERIFY: READ-ONLY SICHERHEITSPRUEFUNG DER
-- AUDIT-PAYLOAD cbb_private_backup.value_add_payload_v4
-- ============================================================================
-- WARUM ES DIESE DATEI GIBT:
--
--   04_verify_read_only.sql prueft die Audit-Payload ausschliesslich
--   INHALTLICH (12 Zeilen, 0 Feldabweichungen gegen public.products). Es prueft
...
--   Payload leistet es diese Datei. Sie ist das Batch-4-Gegenstueck zu
--   ../production_value_add_batch3/04b_verify_payload_security_read_only.sql
...
--      (03_backfill_value_add_batch4.sql) und ergaenzt Schritt 04. Sie ersetzt
...
--   4. Fehlt die Payload-Tabelle cbb_private_backup.value_add_payload_v4,
...
-- ---------------------------------------------------------------------------
payload_presence as (
  select to_regclass('cbb_private_backup.value_add_payload_v4') is not null
...
-- Tabelle cbb_value_add_payload_b4, uebernommen per `create table ... as
-- select *`).
-- ---------------------------------------------------------------------------
payload_shape as (
  select
  ...
  from pg_attribute a
  where a.attrelid = 'cbb_private_backup.value_add_payload_v4'::regclass
    and a.attnum > 0
...
payload_guard as (
  ...
  from pg_class c
  where c.oid = 'cbb_private_backup.value_add_payload_v4'::regclass
),
...
table_privileges_effective as (
  ...
  where r.rolname in ('anon', 'authenticated')
    and has_table_privilege(
          r.oid,
          'cbb_private_backup.value_add_payload_v4'::regclass::oid,
          p.priv::text)
),
schema_privileges_effective as (
  ...
  join pg_class c
    on c.oid = 'cbb_private_backup.value_add_payload_v4'::regclass
  where r.rolname in ('anon', 'authenticated')
    and has_schema_privilege(r.oid, c.relnamespace::oid, p.priv::text)
),

-- ---------------------------------------------------------------------------
-- Artefakte von Batch 1, 2 und 3 muessen unveraendert vorliegen (Pruefung 10)
...
service_role_state as (
  select
    ...
    (select coalesce(string_agg(p.priv, ', ' order by p.priv), 'keine')
     from pg_roles r
     cross join (values ('USAGE'), ('CREATE')) as p(priv)
     join pg_class c on c.oid = 'cbb_private_backup.value_add_payload_v4'::regclass
     where r.rolname = 'service_role'
       and has_schema_privilege(r.oid, c.relnamespace::oid, p.priv::text)
    ) as schema_effektiv,
...
checks as (
  ...
  union all
  -- 1
  select 30, 'payload_v4_vorhanden', payload_vorhanden::text, 'true',
    case when payload_vorhanden then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 2
  select 40, 'payload_v4_zeilen', zeilen::text, '12',
    case when zeilen = 12 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 3
  select 50, 'payload_v4_spalten',
  ...
  union all
  -- 4
  select 60, 'payload_v4_rls', rls::text, 'true',
    case when rls is true then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 5
  select 70, 'payload_v4_policies', policies::text, '0',
    case when policies = 0 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 6
  select 80, 'payload_v4_primaerschluessel',
  ...
  union all
  -- 8
  select 100, 'payload_v4_tabellenrechte_app_rollen',
  ...
  union all
  -- 9
  select 110, 'payload_v4_schemarechte_app_rollen',
    'Rollen ' || app_rollen::text || '/2'
  ...
  select 210, 'service_role_tabellen_acl',
    ...
    'INFO: direkte ACL-Eintraege auf value_add_payload_v4',
    'INFO'
  from summary
  union all
  -- INFO C
