-- ============================================================================
-- PRODUCTION VALUE-ADD BATCH 2 — 04 READ-ONLY NACHPRUEFUNG (INHALT)
-- ============================================================================
-- SICHERHEITSHINWEISE — vor dem Ausfuehren lesen:
--
--   1. Diese Datei laeuft AUSSCHLIESSLICH nach abgeschlossenem Schritt 03
--      (03_backfill_value_add_batch2.sql). Sie prueft den INHALT. Die
...
--   4. Fehlt der Snapshot v2 oder die Payload v2, scheitert das Statement
...
--
-- GRENZE (bewusst, siehe RUNBOOK Abschnitt 8): die Batch-1-Artefakte werden nur
-- ueber to_regclass geprueft, damit ihr Fehlen als lesbare FAIL-Zeile erscheint
-- statt als Planungsfehler. Der inhaltliche Beleg, dass Batch 1 unveraendert
-- ist, kommt aus public.products (Zeilen 90 und 100).
--
...
-- ---------------------------------------------------------------------------
-- Zustand der zehn Batch-2-Zielprodukte (Pruefungen 4, 6, 9, 10, 11, 12)
-- ---------------------------------------------------------------------------
target_state as (
  select
...
-- ---------------------------------------------------------------------------
-- Globalzustand: Batch 1 plus Batch 2, sonst nichts (Pruefungen 7, 8)
-- ---------------------------------------------------------------------------
global_state as (
  select
    (select count(*)
...
-- ---------------------------------------------------------------------------
-- Bewusste Ausnahme: infactory bleibt strukturell relationslos (Pruefung 15)
-- ---------------------------------------------------------------------------
infactory_state as (
  select count(*)::integer as infactory_relationslos
  from public.products p
  where p.slug = 'infactory-boyfriend-kissen'
    and p.alternative_slug is null
    and p.alternative_reason is null
...
-- ---------------------------------------------------------------------------
-- Snapshot v2 und Payload v2 (Pruefungen 3, 5, 14)
-- ---------------------------------------------------------------------------
snapshot_state as (
  select
...
-- ---------------------------------------------------------------------------
-- Schema und Umgebung (Pruefungen 1, 2, 17)
-- ---------------------------------------------------------------------------
schema_state as (
  select
...
checks as (
  ...
  union all
  -- 3
  select 50, 'snapshot_v2_zeilen', snapshot_zeilen::text, '10',
  ...
  select 90, 'value_add_gesamt', value_add_gesamt::text,
    '20 (10 aus Batch 1 + 10 aus Batch 2)',
    case when value_add_gesamt = 20 then 'PASS' else 'FAIL' end from summary
  union all
  -- 8
  select 100, 'batch1_befuellung_intakt', batch1_vollstaendig::text, '10',
    case when batch1_vollstaendig = 10 then 'PASS' else 'FAIL' end from summary
  union all
  -- 9
  ...
  union all
  -- 15
  select 170, 'infactory_bleibt_relationslos', infactory_relationslos::text, '1',
    case when infactory_relationslos = 1 then 'PASS' else 'FAIL' end from summary
  union all
  -- 16
  ...
  select 210, 'relationsliste', relationsliste,
    'INFO: erwartet sind genau zwei Triplets — kaffeewaermer -> gluecksgut'
      || ' (complement) und gluecksgut -> shashibo (alternative)',
    'INFO' from summary
)
