-- ============================================================================
-- PRODUCTION VALUE-ADD BATCH 2 — 04b VERIFY: READ-ONLY SICHERHEITSPRUEFUNG DER
-- AUDIT-PAYLOAD cbb_private_backup.value_add_payload_v2
-- ============================================================================
...
--   Payload leistet es diese Datei. Sie ist das Batch-2-Gegenstueck zu
--   ../production_value_add/05b_verify_payload_security_read_only.sql und
...
--      (03_backfill_value_add_batch2.sql) und ergaenzt Schritt 04. Sie ersetzt
...
--   4. Fehlt die Payload-Tabelle cbb_private_backup.value_add_payload_v2,
--      scheitert das Statement bewusst hart mit "relation does not exist".
--      Das ist der gewollte fail-closed Ausgang: dann ist der Zustand nach 03
...
--   Ist-Zustand ohne jede Zusage. Wer eine Aussage ueber service_role braucht,
--   braucht dafuer einen eigenen, getrennt freigegebenen Vorgang.
--
...
-- Die Namensliste ist die Payload-Definition aus 03 (temporaere Tabelle
-- cbb_value_add_payload_b2, uebernommen per `create table ... as select *`).
-- ---------------------------------------------------------------------------
payload_shape as (
  select
  ...
  from pg_attribute a
  where a.attrelid = 'cbb_private_backup.value_add_payload_v2'::regclass
    and a.attnum > 0
...
-- still 0 Treffer. Ein Zaehler von 0 waere dann KEIN Beleg fuer "keine
...
-- mitgliedschaft geerbte Rechte deckt Teil b ab.
-- ---------------------------------------------------------------------------
table_privileges as (
  select
...
-- ACL-Zaehlern unsichtbar, hier aber sichtbar. Erwartet: jeweils 0.
...
table_privileges_effective as (
  ...
  where r.rolname in ('anon', 'authenticated')
    and has_table_privilege(
          r.oid,
          'cbb_private_backup.value_add_payload_v2'::regclass::oid,
          p.priv::text)
),
schema_privileges_effective as (
  ...
  join pg_class c
    on c.oid = 'cbb_private_backup.value_add_payload_v2'::regclass
  where r.rolname in ('anon', 'authenticated')
    and has_schema_privilege(r.oid, c.relnamespace::oid, p.priv::text)
),

-- ---------------------------------------------------------------------------
-- Batch-1-Artefakte muessen unveraendert vorliegen (Pruefung 10)
...
-- ACL-Eintraege und die effektiven Privilegien (letztere beziehen
-- Mitgliedschaft und PUBLIC-Rechte mit ein). Alle Aggregate sind nach
-- Privilegnamen sortiert, damit die Ausgabe deterministisch ist. Existiert die
-- Rolle nicht, liefern die Unterabfragen 0 Zeilen — has_*_privilege wird dann
...
service_role_state as (
  select
    ...
    (select coalesce(string_agg(p.priv, ', ' order by p.priv), 'keine')
     from pg_roles r
     cross join (values ('USAGE'), ('CREATE')) as p(priv)
     join pg_class c on c.oid = 'cbb_private_backup.value_add_payload_v2'::regclass
     where r.rolname = 'service_role'
       and has_schema_privilege(r.oid, c.relnamespace::oid, p.priv::text)
    ) as schema_effektiv,
...
checks as (
  ...
  union all
  -- 1
  select 30, 'payload_v2_vorhanden', payload_vorhanden::text, 'true',
    case when payload_vorhanden then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 2
  select 40, 'payload_v2_zeilen', zeilen::text, '10',
    case when zeilen = 10 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 3
  select 50, 'payload_v2_spalten',
  ...
  union all
  -- 4
  select 60, 'payload_v2_rls', rls::text, 'true',
    case when rls is true then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 5
  select 70, 'payload_v2_policies', policies::text, '0',
    case when policies = 0 then 'PASS' else 'FAIL' end
  from summary
  union all
  -- 6
  select 80, 'payload_v2_primaerschluessel',
  ...
  union all
  -- 8
  select 100, 'payload_v2_tabellenrechte_app_rollen',
  ...
  union all
  -- 9
  select 110, 'payload_v2_schemarechte_app_rollen',
    'Rollen ' || app_rollen::text || '/2'
  ...
  union all
  -- 10
  select 120, 'batch1_artefakte_vorhanden',
    'snapshot v1 ' || v1_snapshot_da::text
      || ' | payload v1 ' || v1_payload_da::text,
    'snapshot v1 true | payload v1 true',
  ...
  select 210, 'service_role_tabellen_acl',
    ...
    'INFO: direkte ACL-Eintraege auf value_add_payload_v2',
    'INFO'
  from summary
  union all
  -- INFO C
