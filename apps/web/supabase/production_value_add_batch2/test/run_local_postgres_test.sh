#!/usr/bin/env bash
# =============================================================================
# LOKALER POSTGRESQL-HARNESS FUER DEN VALUE-ADD-ROLLOUT — BATCH 2
# =============================================================================
# Testet die ORIGINALDATEIEN 01_preflight_read_only.sql bis
# 05_restore_value_add_batch2.sql aus ../ gegen eine echte PostgreSQL-Instanz
# mit einer production-aehnlichen Fixture (Stand NACH Batch 1).
#
...
# unter ../../production_value_add/ wird veraendert oder ausgefuehrt — der
# Batch-1-Zustand wird von fixture/03_v1_artifacts.sql nachgebaut, und
# CASE 0 belegt per sha256 dass Batch 1 byteweise unangetastet ist.
#
# ZWEISTUFIGER AUFBAU:
#   Stufe 1 (CASE 0) laeuft OHNE Cluster: sha256-Integritaet von Batch 1,
#   SET-LOCAL-Position, begin/commit-Paarigkeit und Read-only-Reinheit der vier
#   lesenden Dateien. Diese Stufe laeuft immer, auch ohne PostgreSQL.
...
SQL_DIR="$(cd "$HERE/.." && pwd)"          # production_value_add_batch2/
REPO_SUPABASE="$(cd "$SQL_DIR/.." && pwd)" # apps/web/supabase/
V1_DIR="$REPO_SUPABASE/production_value_add"

WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/cbb-pgtest-b2.XXXXXXXX")"
PGDATA="$WORKDIR/data"
...
LOCK_APP="cbb_lock_holder_b2_$(basename "$WORKDIR")"
...
# ---------------------------------------------------------------------------
# static_v1_manifest
#   sha256-Beleg, dass KEINE Datei des Batch-1-Changesets veraendert wurde.
#   Deckt alle 34 Dateien unter production_value_add/ plus den gemeinsam
#   genutzten seo_updated_at_trigger.sql ab.
# ---------------------------------------------------------------------------
static_v1_manifest() {
  STEP=$((STEP + 1))
  local out="$LOGDIR/$(printf '%03d' "$STEP")_v1_manifest.log"
  local rc=0
  ( cd "$HERE" && sha256sum -c V1_MANIFEST.sha256 ) > "$out" 2>&1 || rc=$?

  local n_ok n_bad
  n_ok="$(grep -c ': OK$' "$out" || true)"
  n_bad="$(grep -cE ': (FAILED|FEHLGESCHLAGEN)' "$out" || true)"

  if [[ $rc -eq 0 && "$n_bad" -eq 0 && "$n_ok" -ge 34 ]]; then
    record v1_manifest_sha256 "V1_MANIFEST.sha256" "34x OK" "$rc" PASS \
      "$n_ok Dateien byte-identisch, 0 Abweichungen"
  else
    mark_static_fail
    record v1_manifest_sha256 "V1_MANIFEST.sha256" "34x OK" "$rc" FAIL \
...
#       Batch 2 entfernt nichts, auch keine v2-Artefakte. Einzige erlaubte
#       Ausnahme ist die Klausel "on commit drop" der temporaeren Payload-
#       Tabelle in 03: sie raeumt ausschliesslich die Sitzungstabelle ab und
#       kann keine persistente Relation treffen.
#     * jede ausfuehrbare Zeile, die value_add_*_v1 nennt, muss dies innerhalb
#       eines to_regclass()-Aufrufs tun. Damit ist statisch belegt, dass Batch 1
#       ausschliesslich auf Existenz geprueft und nie gelesen oder geschrieben
#       wird.
# ---------------------------------------------------------------------------
static_write_safety() {
  local file="$1"
  ...
  res="$(awk '
    {
      ...
      if (low ~ /value_add_(pre_backfill|payload)_v1/) {
        v1_lines++
        if (low !~ /to_regclass/) { v1_bad++; if (v1_bad_l == 0) v1_bad_l = NR }
      }
    }
    END {
      p = ""
      ...
      if (v1_bad    > 0) p = p sprintf("%d v1-Referenz(en) ohne to_regclass, erste in Z%d; ", v1_bad, v1_bad_l)
      if (p == "")
        printf "OK|0 DROP/TRUNCATE/DELETE (%d x erlaubtes on-commit-drop), %d v1-Referenzen alle via to_regclass\n", oncommit, v1_lines
      else
        printf "PROBLEM|%s\n", p
    }
  ' "$file")"
  ...
  if [[ "$res" == OK\|* ]]; then
    record "$label" "$(basename "$file")" "kein DROP, v1 ro" "-" PASS "${res#OK|}"
  else
    mark_static_fail
    record "$label" "$(basename "$file")" "kein DROP, v1 ro" "-" FAIL "${res#PROBLEM|}"
  fi
}
...
#   uebrig bleiben, und es darf genau EIN Semikolon geben (das Statement-Ende).
#
...
# ---------------------------------------------------------------------------
# static_slug_set
#   Belegt, dass in JEDER der sieben Batch-2-Dateien alle zehn Zielslugs
#   vorkommen und KEIN Batch-1-Slug versehentlich in eine Zielliste gerutscht
#   ist, wo er nicht als Batch-1-Referenz gemeint war. Geprueft wird die
#   Vollstaendigkeit der zehn Slugs — ein vergessener Slug wuerde sonst erst im
#   Cluster auffallen.
# ---------------------------------------------------------------------------
static_slug_set() {
  local file="$1"
  ...
  local s
  for s in livondo-terracotta-pflanzenbewaesserung \
           ...
           katzenschlafsack-fuer-menschen; do
  ...
  if [[ $found -eq 10 ]]; then
    record "$label" "$(basename "$file")" "10/10 Slugs" "-" PASS \
      "alle zehn Batch-2-Slugs vorhanden"
  else
    mark_static_fail
...
head1 "case_0_statisch — Integritaet von Batch 1, SET-LOCAL-Position, Read-only-Reinheit"
CURRENT_CASE=case_0_statisch

static_v1_manifest

static_set_local "$SQL_DIR/02_backup_value_add_batch2.sql"
static_set_local "$SQL_DIR/03_backfill_value_add_batch2.sql"
static_set_local "$SQL_DIR/05_restore_value_add_batch2.sql"

static_write_safety "$SQL_DIR/02_backup_value_add_batch2.sql"
static_write_safety "$SQL_DIR/03_backfill_value_add_batch2.sql"
static_write_safety "$SQL_DIR/05_restore_value_add_batch2.sql"
...
static_slug_set "$SQL_DIR/02_backup_value_add_batch2.sql"
static_slug_set "$SQL_DIR/02b_verify_snapshot_read_only.sql"
static_slug_set "$SQL_DIR/03_backfill_value_add_batch2.sql"
static_slug_set "$SQL_DIR/04_verify_read_only.sql"
static_slug_set "$SQL_DIR/05_restore_value_add_batch2.sql"
...
# -----------------------------------------------------------------------------
# Aufloesung der PostgreSQL-Binaries. Ergebnis ist immer PG_BINDIR.
#   1. CBB_PG_BIN (Prefix mit bin/ ODER direkt das bin-Verzeichnis)
#   2. pg_config --bindir
#   3. Verzeichnis von initdb im PATH
#   4. /usr/lib/postgresql/<major>/bin, falls vorhanden
# -----------------------------------------------------------------------------
PG_BINDIR=""
...
# ---------------------------------------------------------------------------
# report_table_expect_fail <db> <label> <sql-datei> <erwartete-PASS> <erwartete-FAIL> <pflicht-pruefname>
...
step cbb_fixture ok fixture_v1_artefakte "$HERE/fixture/03_v1_artifacts.sql"
...
report_table case_a_happy_path a_01_preflight "$SQL_DIR/01_preflight_read_only.sql" 16
step case_a_happy_path ok a_02_backup   "$SQL_DIR/02_backup_value_add_batch2.sql"
...
step case_a_happy_path ok a_03_backfill "$SQL_DIR/03_backfill_value_add_batch2.sql"
step case_a_happy_path ok a_03_assert   "$HERE/cases/assert_after_03.sql"
step case_a_happy_path ok a_v1_intakt   "$HERE/cases/assert_v1_untouched.sql"
...
step case_b_wiederholungen fail b_02b_vor_backup "$SQL_DIR/02b_verify_snapshot_read_only.sql" \
  'relation "cbb_private_backup.value_add_pre_backfill_v2" does not exist'
step case_b_wiederholungen fail b_03_vor_backup  "$SQL_DIR/03_backfill_value_add_batch2.sql" \
  'Batch-2-Backfill abgebrochen: privater Snapshot v2 fehlt.'
step case_b_wiederholungen fail b_05_vor_backup  "$SQL_DIR/05_restore_value_add_batch2.sql" \
  'Batch-2-Restore abgebrochen: privater Snapshot v2 fehlt.'
step case_b_wiederholungen ok   b_02_backup      "$SQL_DIR/02_backup_value_add_batch2.sql"
step case_b_wiederholungen fail b_02_wiederholung "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: Snapshot v2 existiert bereits.'
...
step case_b_wiederholungen fail b_04_vor_backfill "$SQL_DIR/04_verify_read_only.sql" \
  'relation "cbb_private_backup.value_add_payload_v2" does not exist'
step case_b_wiederholungen fail b_04b_vor_backfill "$SQL_DIR/04b_verify_payload_security_read_only.sql" \
  'relation "cbb_private_backup.value_add_payload_v2" does not exist'
step case_b_wiederholungen ok   b_03_backfill     "$SQL_DIR/03_backfill_value_add_batch2.sql"
# Nach einem erfolgreichen 03 schlaegt der PAYLOAD-Guard zu, nicht der
# Drift-Guard: die Existenzpruefung von value_add_payload_v2 steht bewusst im
# ersten Guard-Block und damit vor dem Zeilen-Lock. Der Drift-Guard wird in
# case_n_drift eigens geprueft.
step case_b_wiederholungen fail b_03_wiederholung "$SQL_DIR/03_backfill_value_add_batch2.sql" \
  'Batch-2-Backfill abgebrochen: Audit-Payload v2 existiert bereits.'
step case_b_wiederholungen ok   b_03_nach_abbruch "$HERE/cases/assert_after_03.sql"
step case_b_wiederholungen ok   b_v1_intakt       "$HERE/cases/assert_v1_untouched.sql"

# ===========================================================================
# CASE N — Drift zwischen Snapshot und Bestand
# ===========================================================================
new_case case_n_drift "Zielzeile aendert sich nach dem Snapshot"
step case_n_drift ok   n_02_backup   "$SQL_DIR/02_backup_value_add_batch2.sql"
step case_n_drift ok   n_setup_drift "$HERE/cases/setup_drift_after_snapshot.sql"
step case_n_drift fail n_03_abbruch  "$SQL_DIR/03_backfill_value_add_batch2.sql" \
  'Batch-2-Backfill abgebrochen: 1 Zielzeilen sind seit dem Snapshot gedriftet.'
step case_n_drift ok   n_assert      "$HERE/cases/assert_drift_unchanged.sql"
step case_n_drift ok   n_v1_intakt   "$HERE/cases/assert_v1_untouched.sql"
...
report_table_expect_fail case_n_drift n_02b_meldet_drift \
  ...
  'snapshot_v2_gegen_products_drift'

# ===========================================================================
# CASE C — Transaktions-Rollback mitten in 03
# ===========================================================================
new_case case_c_rollback "Abbruch nach dem Payload-DDL in 03"
step case_c_rollback ok   c_02_backup         "$SQL_DIR/02_backup_value_add_batch2.sql"
step case_c_rollback ok   c_block_installieren "$HERE/cases/setup_block_updates.sql"
step case_c_rollback fail c_03_bricht_ab      "$SQL_DIR/03_backfill_value_add_batch2.sql" \
...
step case_c_rollback ok   c_v1_intakt         "$HERE/cases/assert_v1_untouched.sql"
step case_c_rollback ok   c_block_entfernen   "$HERE/cases/teardown_block_updates.sql"
step case_c_rollback ok   c_03_danach_ok      "$SQL_DIR/03_backfill_value_add_batch2.sql"
...
step case_d_restore_roundtrip ok   d_02_backup       "$SQL_DIR/02_backup_value_add_batch2.sql"
step case_d_restore_roundtrip ok   d_03_backfill     "$SQL_DIR/03_backfill_value_add_batch2.sql"
step case_d_restore_roundtrip ok   d_05_restore      "$SQL_DIR/05_restore_value_add_batch2.sql"
step case_d_restore_roundtrip ok   d_05_assert       "$HERE/cases/assert_after_05.sql"
step case_d_restore_roundtrip ok   d_05_wiederholung "$SQL_DIR/05_restore_value_add_batch2.sql"
step case_d_restore_roundtrip ok   d_05_assert_2     "$HERE/cases/assert_after_05.sql"
step case_d_restore_roundtrip fail d_03_nach_restore "$SQL_DIR/03_backfill_value_add_batch2.sql" \
  'Batch-2-Backfill abgebrochen: Audit-Payload v2 existiert bereits.'
step case_d_restore_roundtrip ok   d_05_assert_3     "$HERE/cases/assert_after_05.sql"
step case_d_restore_roundtrip ok   d_v1_intakt       "$HERE/cases/assert_v1_untouched.sql"
...
step case_e_pilot_artefakt fail e_02_abbruch "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: Pilot-Artefakt gefunden.'
step case_e_pilot_artefakt ok   e_assert     "$HERE/cases/assert_no_batch2_artifacts.sql"
step case_e_pilot_artefakt ok   e_v1_intakt  "$HERE/cases/assert_v1_untouched.sql"
...
step case_f_zu_wenig_produkte fail f_02_abbruch "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: nur 299 Produkte (< 300).'
step case_f_zu_wenig_produkte ok   f_assert     "$HERE/cases/assert_no_batch2_artifacts.sql"
step case_f_zu_wenig_produkte ok   f_v1_intakt  "$HERE/cases/assert_v1_untouched.sql"
...
step case_g_unvollstaendiges_schema fail g_02_abbruch "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: Value-Add-Schema unvollstaendig (7 Spalten, 7 Typen, 2 Constraints).'
...
step case_h_relationsziel_offline fail h_02_abbruch "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: 9/10 Zielprodukte published.'
step case_h_relationsziel_offline ok   h_assert     "$HERE/cases/assert_no_batch2_artifacts.sql"
step case_h_relationsziel_offline ok   h_v1_intakt  "$HERE/cases/assert_v1_untouched.sql"
...
step case_m_fremdbefuellung fail m_02_abbruch "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: 1 Zielprodukte enthalten bereits Value-Add-Daten.'
step case_m_fremdbefuellung ok   m_assert     "$HERE/cases/assert_prefill_unchanged.sql"
step case_m_fremdbefuellung ok   m_v1_intakt  "$HERE/cases/assert_v1_untouched.sql"

# ===========================================================================
# CASE K — Batch-1-Artefakt fehlt
# ===========================================================================
new_case case_k_batch1_artefakt_fehlt "value_add_payload_v1 geloescht"
step case_k_batch1_artefakt_fehlt ok   k_setup      "$HERE/cases/setup_drop_v1_payload.sql"
step case_k_batch1_artefakt_fehlt fail k_02_abbruch "$SQL_DIR/02_backup_value_add_batch2.sql" \
  'Batch-2-Backup abgebrochen: Batch-1-Payload v1 fehlt.'
step case_k_batch1_artefakt_fehlt ok   k_assert     "$HERE/cases/assert_v1_payload_missing.sql"
...
step case_l_payload_security ok l_02_backup   "$SQL_DIR/02_backup_value_add_batch2.sql"
step case_l_payload_security ok l_03_backfill "$SQL_DIR/03_backfill_value_add_batch2.sql"
...
report_table_expect_fail case_l_payload_security l_04b_nachher \
  ...
  'payload_v2_tabellenrechte_app_rollen'
...
# dass value_add_payload_v2 noch NICHT existiert. Nach einem Backfill waere
...
step case_o_snapshot_security ok l_02_backup_o "$SQL_DIR/02_backup_value_add_batch2.sql"
...
report_table_expect_fail case_o_snapshot_security o_02b_nachher \
  ...
  'snapshot_v2_tabellenrechte_app_rollen'
...
step case_j_lock_timeout ok j_vorbereitung "$HERE/cases/assert_no_batch2_artifacts.sql"
...
if [[ $LOCK_HELD -ne 1 ]]; then
  mark_fail
  record j_02_unter_sperre "02_backup_value_add_batch2.sql" "fail/3" "-" FAIL \
    "Sperre konnte nicht aufgebaut werden — Fall nicht bewertbar"
else
  J_START=$SECONDS
  timeout 30 "$PSQL" -X -q -v ON_ERROR_STOP=1 -d case_j_lock_timeout \
    -f "$SQL_DIR/02_backup_value_add_batch2.sql" > "$J_OUT" 2>&1 &
  J_PSQL_PID=$!
  ...
  if [[ $J_RC -eq 124 ]]; then
    mark_fail
    record j_02_unter_sperre "02_backup_value_add_batch2.sql" "fail/3" "$J_RC" FAIL \
    ...
    record j_02_unter_sperre "02_backup_value_add_batch2.sql" "fail/3" "$J_RC" PASS \
...
step case_j_lock_timeout ok j_assert_unveraendert "$HERE/cases/assert_no_batch2_artifacts.sql"

# Der Nachtest darf nicht auf das Ende von pg_sleep(90) warten.
N_START=$SECONDS
step case_j_lock_timeout ok j_02_danach "$SQL_DIR/02_backup_value_add_batch2.sql"
