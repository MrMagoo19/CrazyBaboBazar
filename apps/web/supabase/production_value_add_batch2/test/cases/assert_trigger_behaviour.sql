...
begin
  insert into public.products (slug, name, tagline, affiliate_url, is_published,
                               updated_at)
  values ('cbb-test-trigger-probe-b2', 'Trigger-Probe', 'Vorher',
          'https://example.invalid/aff/probe', true,
          timestamptz '2026-01-01 00:00:00+00');

  select updated_at into t0 from public.products
  where slug = 'cbb-test-trigger-probe-b2';
  ...
  --    verlaesst sich 05_restore_value_add_batch2.sql beim Zurueckspielen.
  update public.products
  ...
  delete from public.products where slug = 'cbb-test-trigger-probe-b2';
