...
interface HeroLandingProps {
  ...
  loginText?: string
  loginHref?: string
  /** Set to false to hide the built-in header/nav (e.g. when the parent layout provides navigation) */
  showNav?: boolean
  title: string
  description: string
...
export function HeroLanding(props: HeroLandingProps) {
  ...
  return (
    <div className={`min-h-screen w-screen overflow-x-hidden relative ${className || ''}`}>
      ...
      {showNav && <header className="absolute inset-x-0 top-0 z-10">
      ...
      <div className="relative isolate px-6 pt-4 overflow-hidden min-h-screen flex flex-col justify-center">
        <div className={`mx-auto max-w-4xl ${showNav ? 'pt-20 sm:pt-25' : 'pt-8'}`}>
          {announcementBanner && (
